    ...
    try {
      ...
      try {
        ...
        const templateResponse = await fetch(
          `/api/session/${this.sessionId}/template/${encodedTemplatePathForContent}`,
        );
      ...
      if (
        ...
        this.currentData.categories.compodocConfig &&
        this.currentData.categories.compodocConfig.data
      ) {
...
<p><strong>Type:</strong> ${templatePath}</p>`
  ...
  updateTemplateMetadata(templatePath, data) {
    ...
    document.getElementById('templateDescription').textContent =
      data.description || `Template for ${templatePath}`;
      ...
      if (result.success) {
        ...
        if (iframe) {
          ...
          if (this.lastVisitedDocUrl) {
            ...
            let docPath = this.lastVisitedDocUrl.replace(
              new RegExp(`^/docs/${this.sessionId}/?`),
              '',
            );
  ...
  setPreviewStatus(text, isLoading) {
    ...
    statusElement.innerHTML = isLoading ? `<div class="spinner"></div> ${text}` : text;
  }
  ...
  clearTemplate() {
    ...
    if (iframe) {
      iframe.src =
        'data:text/html,<div style="padding: 20px; text-align: center; color: #666;"><div style="font-size: 18px; margin-bottom: 10px;">📝</div>Select a template to see preview</div>';
    }
  }
