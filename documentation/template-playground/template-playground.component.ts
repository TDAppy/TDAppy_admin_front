    ...
    <div class="template-playground">
      <div class="template-playground-header">
        ...
        <div class="template-playground-status">
          <span *ngIf="sessionId" class="session-info"
            >Session: {{ sessionId.substring(0, 8) }}...</span
          >
          <span *ngIf="saving" class="saving-indicator">Saving...</span>
          <span *ngIf="lastSaved" class="last-saved"
            >Last saved: {{ lastSaved | date: 'short' }}</span
          >
        </div>
      ...
      <div class="config-panel" [class.collapsed]="!showConfigPanel">
        ...
        <div class="config-options">
          <label
            ><input type="checkbox" [(ngModel)]="config.hideGenerator" (change)="updateConfig()" />
            Hide Generator</label
          >
          <label
            ><input
              type="checkbox"
              [(ngModel)]="config.hideDarkModeToggle"
              (change)="updateConfig()"
            />
            Hide Dark Mode Toggle</label
          >
          <label
            ><input type="checkbox" [(ngModel)]="config.minimal" (change)="updateConfig()" />
            Minimal Mode</label
          >
          <label
            ><input
              type="checkbox"
              [(ngModel)]="config.disableOverview"
              (change)="updateConfig()"
            />
            Disable Overview</label
          >
          <label
            ><input
              type="checkbox"
              [(ngModel)]="config.disableFilePath"
              (change)="updateConfig()"
            />
            Disable File Path</label
          >
          <label
            ><input
              type="checkbox"
              [(ngModel)]="config.disableSourceCode"
              (change)="updateConfig()"
            />
            Disable Source Code</label
          >
          <label
            ><input type="checkbox" [(ngModel)]="config.disableGraph" (change)="updateConfig()" />
            Disable Graph</label
          >
          <label
            ><input
              type="checkbox"
              [(ngModel)]="config.disableMainGraph"
              (change)="updateConfig()"
            />
            Disable Main Graph</label
          >
          <label
            ><input
              type="checkbox"
              [(ngModel)]="config.disableRoutesGraph"
              (change)="updateConfig()"
            />
            Disable Routes Graph</label
          >
          <label
            ><input
              type="checkbox"
              [(ngModel)]="config.disableCoverage"
              (change)="updateConfig()"
            />
            Disable Coverage</label
          >
          <label
            ><input type="checkbox" [(ngModel)]="config.disableSearch" (change)="updateConfig()" />
            Disable Search</label
          >
          <label
            ><input
              type="checkbox"
              [(ngModel)]="config.disableDependencies"
              (change)="updateConfig()"
            />
            Disable Dependencies</label
          >
          <label
            ><input type="checkbox" [(ngModel)]="config.disablePrivate" (change)="updateConfig()" />
            Disable Private</label
          >
          <label
            ><input
              type="checkbox"
              [(ngModel)]="config.disableProtected"
              (change)="updateConfig()"
            />
            Disable Protected</label
          >
          <label
            ><input
              type="checkbox"
              [(ngModel)]="config.disableInternal"
              (change)="updateConfig()"
            />
            Disable Internal</label
          >
          <label
            ><input
              type="checkbox"
              [(ngModel)]="config.disableLifeCycleHooks"
              (change)="updateConfig()"
            />
            Disable Lifecycle Hooks</label
          >
          <label
            ><input
              type="checkbox"
              [(ngModel)]="config.disableConstructors"
              (change)="updateConfig()"
            />
            Disable Constructors</label
          >
          <label
            ><input
              type="checkbox"
              [(ngModel)]="config.disableProperties"
              (change)="updateConfig()"
            />
            Disable Properties</label
          >
          <label
            ><input type="checkbox" [(ngModel)]="config.disableDomTree" (change)="updateConfig()" />
            Disable DOM Tree</label
          >
          <label
            ><input
              type="checkbox"
              [(ngModel)]="config.disableTemplateTab"
              (change)="updateConfig()"
            />
            Disable Template Tab</label
          >
          <label
            ><input
              type="checkbox"
              [(ngModel)]="config.disableStyleTab"
              (change)="updateConfig()"
            />
            Disable Style Tab</label
          >
        </div>
      </div>
      ...
      <div class="template-playground-body">
        <div class="template-playground-sidebar">
          <div class="template-file-list">
            ...
            <ul class="file-list">
              <li
                *ngFor="let template of templates; trackBy: trackByName"
            ...
            <div *ngIf="templates.length === 0" class="loading-templates">Loading templates...</div>
          </div>
        </div>
...
export class TemplatePlaygroundComponent implements OnInit, OnDestroy {
  ...
  private async loadSessionTemplates(): Promise<void> {
    ...
    const response = await this.http
      .get<{ templates: Template[]; success: boolean }>(`/api/session/${this.sessionId}/templates`)
      .toPromise();
    if (response && response.success) {
  ...
  private async loadSessionConfig(): Promise<void> {
    ...
    const response = await this.http
      .get<{ config: CompoDocConfig; success: boolean }>(`/api/session/${this.sessionId}/config`)
      .toPromise();
    if (response && response.success) {
    ...
    try {
      const response = await this.http
        .get<{
          content: string;
          success: boolean;
        }>(`/api/session/${this.sessionId}/template/${template.path}`)
        .toPromise();
      if (response && response.success) {
        this.editorService.setEditorContent(
          response.content,
          template.type === 'template' ? 'handlebars' : 'handlebars',
        );
      }
    } catch (error) {
  ...
  private async saveTemplate(content: string): Promise<void> {
    ...
    const response = await this.http
      .post<{ success: boolean }>(
        `/api/session/${this.sessionId}/template/${this.selectedFile.path}`,
        {
          content,
        },
      )
  ...
  async updateConfig(): Promise<void> {
    ...
    try {
      const response = await this.http
        .post<{ success: boolean }>(`/api/session/${this.sessionId}/config`, {
          config: this.config,
        })
        .toPromise();
  ...
  resetToDefault(): void {
    ...
    if (
      confirm(
        'Are you sure you want to reset all templates to their default values? This action cannot be undone.',
      )
    ) {
  ...
  async exportZip(): Promise<void> {
    try {
      ...
      const response = await this.http
        .post(
          `/api/session/${this.sessionId}/download-all-templates`,
          {},
          {
            responseType: 'blob',
            observe: 'response',
